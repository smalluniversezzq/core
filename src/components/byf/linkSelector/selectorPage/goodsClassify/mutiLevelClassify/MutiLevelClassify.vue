<template>
  <div class="muti-level-classify-box" :bodyStyle="{padding:0,'padding-left': '24px','padding-right':'24px'}">
    <div class="header">
      <div class="name-column">分类名称</div>
      <div class="img-column">分类图片</div>
      <div class="operate-column">操作</div>
    </div>
    <div class="main-content">
      <div id="list-box" class="list-box" v-show="realList.length > 0">
        <line-item @checkItem="checkItem" 
          :actived="actived(item)"
          :links="selectedLinks"
    ...
    initData(list, paths = ''){
      list.forEach((item, index) => {
        ...
        if (item.subs.length > 0) {
          this.initData(item.subs, path)
          // 判断子级是否有选中，如果有则展开
          item.subs.forEach(subItem => {
            if(this.actived(subItem)){
              item.showChildren = true;
            }
          })
        }
        this.$set(item, 'link_id', item.link_id + path)
      })
    },
    initSortable(){
