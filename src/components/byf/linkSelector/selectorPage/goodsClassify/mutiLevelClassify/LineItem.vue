  ...
  <div class="line" :class="'level-'+level">
    <div class="line-info">
      <div class="name-column">
        ...
        <!-- <a-input disabled class="name-input" placeholder="请输入分类名" :value="classifyData.title" @change="(e)=>(onInputChange(e))">
          <span slot="suffix">{{classifyData.title.length}}/5</span>
        </a-input> -->
      </div>
      <div class="img-column">
        <div class="img-classify">
          <img :src="classifyData.image">
        </div>
      </div>
      <div class="operate-column">
        <a-button :type="actived ? 'primary' : ''" @click="checkItem(classifyData)">选择</a-button>
      </div>
    </div>
    <div :id="'box'+indexPath.join('')" class="children-box" v-if="classifyData.showChildren && classifyData.subs.length > 0">
      <line-item v-for="(item, index) in classifyData.subs" 
        @checkItem="checkItem"
        :actived="activedItem(item)"
        :links="selectedLinks"
        :key="(level+1)+'item'+index" 
        :classifyData="item" 
        :level="level+1" 
        :indexPath="[...indexPath, index]" 
        @operate="onChildOperate"></line-item>
    </div>
  </div>
</template>
...
export default {
  ...
  watch: {
    links(val){
      this.selectedLinks = cloneDeep(val);
    }
  },
  data(){
    return {
