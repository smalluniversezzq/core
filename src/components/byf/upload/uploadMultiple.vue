<template>
  <div class="byf-upload">
    <!-- 图片列表 -->
    <ul v-if="imgObj.length>0" class="img-list" >
      <li v-for="(item, index) in imgObj" @click="preview" :key="'img'+index" :style="colorType()" :class="size">
        <img :src="item" alt="" >
    ...
    <!-- 添加图片的li -->
    <div v-if="imgObj.length < maximum"
      class="upload"
      :class="size"
      @click="openModal"
    >
...
export default {
  ...
  methods: {
    isShowPreview(array){
      console.log(array,"array",array.length > this.maximum)
      return array.length < this.maximum
    ...
    openModal() {
      this.$byfUpload({multiple:this.multiple,maximum:this.maximum, imgsList: this.imgObj, format:this.format}).then(res => {
