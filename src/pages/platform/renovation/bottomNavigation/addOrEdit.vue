<template>
  ...
  >
    <a-form-model ref="form" :model="form" :rules="rules" 
      ...
      <a-form-model-item v-show="form.device_type == 1" label="适用设备" prop="apply_os">
        <a-checkbox-group
      ...
      <a-form-model-item label="适用版本" prop="version_start" help="说明：可以设置当前模板仅在某个APP版本中使用，版本号格式为1.2.10">
        <a-radio-group v-model="version_type" style="margin-top: 9px;">
          ...
          <ul class="update-logs" v-if="version_type != 0">
            <li v-if="version_type == 1">
              ...
              <div class="dot-wrap">.</div>
              <div class="input"><a-input-number v-model="form.version_start[2]" :min="1" :max="99" :step="1"/></div>
            </li>
            ...
            <li v-if="version_type == 2">
              ...
              <div class="dot-wrap">.</div>
              <div class="input"><a-input-number v-model="form.version_end[2]" :min="1" :max="99" :step="1"/></div>
            </li>
          </ul>
        </a-radio-group>
      </a-form-model-item>
...
<script>
import {getAdminPlatformDesignNavAdd, getAdminPlatformDesignNavSetting} from '@/services/admin/platform/design'
      ...
      this.$refs.form.validate(valid => {
        if (valid) {
          ...
          if (this.operationType == 'add') {
            // 新增
            getAdminPlatformDesignNavAdd(data).then((res) => {
              if (res.code == 200) {
            ...
            // 修改
            getAdminPlatformDesignNavSetting(data).then((res) => {
              if (res.code == 200) {
                this.$message.success(res.msg)
                this.$emit('confirm', {operationType: this.operationType})
                this.modalCancel();
