  ...
  <div class="line" :class="'level-'+level">
    <div class="line-info">
      <div class="name-column">
        ...
        <a-input class="name-input" placeholder="请输入名称" :value="classifyData.title" @change="(e)=>(onInputChange(e))">
          <span slot="suffix">{{classifyData.title.length}}/5</span>
        </a-input>
        <a-button v-if="level < 1" type="link" @click="addChild">+添加导航按钮</a-button>
      </div>
      <div class="img-column">
        <choose-img v-if="level == 1" :imgsList="[classifyData.image]" @change="onImgChange"></choose-img>
      </div>
...
export default {
  name: 'LineItem',
  components: {ChooseImg, linkInput},
  props: {
    /**
     * 一条分类数据
     */
  ...
  mounted(){
    if(this.classifyData.link.link_id){
      this.links = [this.classifyData.link];
    }
    this.initSortable();
  },
  methods: {
    linkConfirm(links){
      this.links = links;
      this.classifyData.link = this.links[0] || {}
    },
    initSortable(){
    ...
    */
    ...
    onImgChange(list){
      if(list.length > 0){
        this.classifyData.image.url = list[0].url;
        this.classifyData.image.size = [list[0].extend.width, list[0].extend.height];
      }else{
        this.classifyData.image.url = '';
        this.classifyData.image.size = [];
      }
    },
    getTime(time){
      if(!time) {
