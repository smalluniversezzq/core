      ...
      <a-spin :spinning="modalSpinning">
        <a-form-model
          ...
          >
            ...
            <a-form-model-item label="AccessKey" prop="storage_oss_accesskeyid">
              <a-input placeholder="请输入AccessKey" v-model="formData.storage_oss_accesskeyid" />
            </a-form-model-item>
            <a-form-model-item label="AccessKey Secret" prop="storage_oss_accesskeysecret">
              <a-input placeholder="请输入AccessKey Secret" v-model="formData.storage_oss_accesskeysecret" />
            </a-form-model-item>
            <a-form-model-item label="地域节点EndPoint" prop="storage_oss_endpoint">
              <a-input placeholder="请输入地域节点EndPoint" v-model="formData.storage_oss_endpoint" />
            </a-form-model-item>
            <a-form-model-item label="存储空间名称Bucket" prop="storage_oss_bucket">
              <a-input placeholder="请输入存储空间名称Bucket" v-model="formData.storage_oss_bucket" />
            </a-form-model-item>
        </a-form-model>
      </a-spin>
    </a-modal>
...
export default {
  ...
  methods: {
    ...
    getStatus(){
      this.spinning = true;
      platformConfigGet({config: ['storage_drive']}).then(res => {
        if(res.code == 200){
          this.typeList.forEach((item) => {
            if(item.storage_drive == res.data.config.storage_drive){
              item.status = true;
            }
          })
        }
      ...
      this.$confirm({
        ...
        onOk: () => {
          return new Promise((resolve, reject) => {
            let storage_drive = "";
            if(!item.status){
              storage_drive = item.storage_drive
            }
            platformConfigSet({storage_drive: storage_drive}).then(res => {
              resolve();
              if(res.code == 200){
                if(!this.statusShow(res.data.result)){
                  ...
                  this.typeList.forEach(val => {
                    if(val.storage_drive != item.storage_drive){
                      val.status = false;
                    }
                  })
