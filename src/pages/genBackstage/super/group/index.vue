<template>
  <div>
      ...
      >
        <a-row :gutter="48">
          <a-col :lg="12" :xl="8" :xxl="6">
            <a-form-model-item label="套餐组名称" prop="name">
              <a-input v-model="searchFormData.name" placeholder="请输入套餐组名称" ></a-input>
            </a-form-model-item>
          </a-col>
          <a-col :lg="12" :xl="8" :xxl="6">
            <a-form-model-item label="状态" prop="status" ref="test">
              <a-select placeholder="请选择状态" v-model="searchFormData.status" 
    ...
    <group-table ref="table" :condition="searchFormData"></group-table>
  </div>

</template>
<script>
import GroupTable from './table'
let defaultSearchFormData = { status: '', name: undefined }
