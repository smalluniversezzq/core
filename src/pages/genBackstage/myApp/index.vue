  ...
  <a-card>
    ...
    >
      ...
      <a-space class="header-right">
        ...
        <a-button type="primary" @click="goCreate" :disabled="user.platform_remain == 0">添加平台</a-button>
      </a-space>
    </a-row>
...
import { getAdminAppList } from "@/services/platform";
...
export default {
  ...
  data() {
    return {
      ...
      selectedSort: '',
      listSpinning: false,  // 加载列表时的spinning
    };
  },
  created() {
    this.getAdminAppList(this.pagination);
  ...
  methods: {
    ...
    getAdminAppList(pageData) {
      console.log(this.pagination)
      this.listSpinning = true
      getAdminAppList({page: pageData.current, page_size: pageData.pageSize, ...this.listParams}).then(res => {
        if (res.code == 200) {
